...
export async function getStaticProps(context) {
     ...
     */
    const { categorieId } = context.params
    //https://docs.mongodb.com/manual/tutorial/query-arrays/
    const products_query = await getProducts({ categories: { $all: categorieId } })
    ...
    const objectCategorieInArray = categs.filter(categ => categ._id === categorieId)
    //console.log('*******************', categs)
    ...
    //Obtenemos los productos asociados al categorieId/categoría que obtenemos del path (context.params) 
    /*
...
const filters = level === 1 ?
    ...
    oneProduct.filters
*/
    // const lastCategorieId = categoriesId.slice(-1)
    //const [lastCategorieId] = categoriesId
    //const childs_query = await getCats({ _id: lastCategorieId })
