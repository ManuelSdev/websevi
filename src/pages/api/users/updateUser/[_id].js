...
export default async function handler(req, res) {
  ...
  //TODO: revisa...hago parse porque ahora no lo hace auto
  ...
  try {
    ...
    const { addresses: currentAddresses } = await User.findById(
      { _id },
      "addresses",
    ).exec();
    ...
    if (currentAddresses.length === 0) {
      ...
      const updatedUser = await User.findByIdAndUpdate(
        { _id },
        { ...newValues, $push: { addresses: newAddress } },
        {
          new: true,
    ...
    //Si la dirección entrante se señaló como default en el formulario, crea un nuevo array de direcciones actuales con todas ellas con defaultAddress=false
      ...
      const currentAddressesNoDefault = currentAddresses.mapp(
        (addressObject = () => {
    ...
    res.status(201).json({ done: true, message: "Usuario actualizado" });
  } catch (err) {
